...
public class Backstage implements ThreadCallBack {
	 ...
	 * 情况分为：
	 * 1. 账号密码与数据表记录相同，'isLogin' 标记未登录，更改标记并返回 true。
	 * 2. 账号密码与数据表记录相同，'isLogin' 标记已登录，广播询问登录情况，视广播情况返回 true 或 false。（有点难度）
	 * 3. 账号对应密码不相同，返回 false。
	 * 4. 不存在此账号信息，添加到数据表、置 'isLogin' 标记并返回 true。（即新建账号操作）
